...
from util.atomic import AtomicEdit
...
def test_lock_state() -> None:
    """
    The lock file exists within the "with" block, but not outside (if single-
    process).
    """
...
def test_lock_cleared_error() -> None:
    """
    Even if an error occurs during the "with" block, the lock should be cleared.
    """
...
def test_second_attempt_waits_for_first() -> None:
    """
...
def test_partial_write(monkeypatch) -> None:
    """
    ...
    function that intentionally breaks after the first line is written. In the
    real-world, it could be an issue for something like half of a JSON file to
    be written, with a parser expecting to read an entire valid JSON file.
    """

    with open(FILE_PATH, "w") as f:
        f.write("original 1\noriginal 2")

    # Replace the tempfile write with a version that breaks after half of the
    # content is written.
...
class TestMultiprocess:
    def worker():
        # The timeout is *not* for this specific worker; it's checking that the
        # lock has been modified at all. So it's fine if another process
    ...
    def test_multiprocess(self: "TestMultiprocess") -> None:
        """
        Tests a large number of processes attempting to write to the file. This
        looks like what a real program would do here.
        """
