...
def worker(job_value: int) -> str:
    ...
    try:
        ...
        art = Artifact(
            ...
            props={"input": job_value, "output": result},
        )
...
class TestIntegration:
    ...
    def test_integration(self: "TestIntegration"):
        ...
        # here.
        ignore_subset = set(
            random.randrange(NUM_JOBS) for _ in range(MISSING_SUBSET_SIZE)
        )
        jobs = [idx for idx in range(NUM_JOBS) if idx not in ignore_subset]
        ...
        assert all(len(r) == 0 for r in results), "\n".join(
            f"{idx}: {r}" for idx, r in zip(jobs, results)
        )
